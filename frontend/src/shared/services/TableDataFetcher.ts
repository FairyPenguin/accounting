import { ApiURLWithPaginationQueryParam } from "@/shared/constants/APIURLs";
import JWT_TOKEN from "../constants/Tokens";
...
export async function tableDataFetcher<DataType>(
    page: number,
    limit: number,
    endpoint: string,
    search: string,
    ...
    try {
        const response = await fetch(`${ApiURLWithPaginationQueryParam(page, limit, endpoint, search, searchByAttribute, lookupType)}`, {
            cache: "no-store",
            headers: {
